...
class BufferReader(object):
    ...
    def read(self, size: int) -> bytes:
        if self.offset + size > len(self.buffer):
            raise InvalidNdef(
                "not enough bytes [offset=%u, len=%u, need=%u]"
                % (self.offset, len(self.buffer), size)
            )
        ...
        if self.tnf == TNF_WELL_KNOWN:
            if self.type == RTD_TEXT:
                ...
                if self.payload_len < 1 + language_len:
                    raise InvalidNdefRecord(
                        "RTD_TEXT contains invalid language code length"
                    )
                ...
                try:
                    ...
                    raise InvalidNdefRecord(
                        "RTD_TEXT contains language code with invalid encoding"
                    )
                ...
                try:
                    ...
                    raise InvalidNdefRecord(
                        "RTD_TEXT payload failed to decode as " + encoding
                    )
                ...
                if self.payload[0] > RTD_URI_ABBRIV_NUM:
                    raise InvalidNdefRecord(
                        "RTD_URI payload starts with an invalid URI identifier code"
                    )
...
def new_message(*record_defs) -> NdefMessage:
    ...
    for record_def in record_defs:
        if len(record_def) != 4:
            raise InvalidNdefRecord(
                "invalid record definition - wrong length [%d but should be 4]"
                % len(record_def)
            )
        record = NdefRecord()
...
def new_smart_poster(title: str, url: str) -> NdefMessage:
    ...
    if title:
        records.append(
            (TNF_WELL_KNOWN, RTD_TEXT, b"", b"\x02en" + title.encode("utf-8"))
        )
